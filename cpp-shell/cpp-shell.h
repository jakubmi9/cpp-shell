...
namespace sh
{
	...
	public:
		...
		~shell();
		///<summary>starts primary shell that takes user input. that input can be found in the [std::vector &lt;string&gt; command] property. should be run in an endless loop, if user does not provide enough input, substart() should be used to get the additional input</summary>
		void start();
